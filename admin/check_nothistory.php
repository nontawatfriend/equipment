...
<head>
<meta charset="utf-8">
<title>Check_history</title>
</head>
<body>
<h3 align="center"><i class="fa fa-th-list" style="font-size:20px"></i> ประวัติการยืม</h3>
<hr>
...
?>
 ...
 <!--<td><i class="fa fa-check"></i> <?=$row["status_name"]?></td>-->
		<?php
		if($row["status_id"] == "2"){
...
<?php
 ...
 ?>
		...
		<div class="form-group">
           <label class="col-sm-6 control-label">ผู้ไม่อนุมัติ :</label>
             <div class="col-sm-4">
				 <p class="form-control-static" value="<?=$row["usernotapprove_id"]?>"><?=$row["usernotapprove_id"]?></p>
             </div>
         </div>
<?php
if($row["notapproved_note"]!=""){
?>
				<div class="form-group">
           			<label class="col-sm-6 control-label">หมายเหตุไม่อนุมัติ :</label>
             			<div class="col-sm-4">
							<p class="form-control-static" rows="3" disabled><?=$row["notapproved_note"]?></p>
             			</div>
         		</div>
<?php }else{?>
						<div class="form-group ">
